...
extension XCTestCase {
    ...
    func loadJSONResponseFromCassetteWithName(_ name: String) -> JSON {
        
        let dictionary = self.loadJSONWithName(name)
        
        let interactions = dictionary["interactions"] as! [JSON]
        let response = interactions.first!["response"] as! JSON
    ...
    func loadJSONWithName(_ name: String) -> JSON {
        ...
        do {
            ...
            if let url = bundle.url(forResource: name, withExtension: "json") {
                ...
                if let json = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions()) as? JSON {
                    return json
                }
